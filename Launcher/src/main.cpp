...
fs::path copyDllToTemp(const fs::path& dllPath);
fs::path getTempDllPath(const fs::path& dllPath);

int main(void) {
    // Find the first DLL in the current directory
    fs::path dllPath = findDLL();
    if (dllPath.empty()) {
        ERROR("No DLL found in current directory");
        return 1;
    }

    INFO("DLL found: `" + dllPath.filename().string() + "`");
    ...
    fs::path tempDllPath = getTempDllPath(dllPath);
    ...
    // Copy the DLL to the temp directory for safer injection
    tempDllPath = copyDllToTemp(dllPath);
    if (tempDllPath.empty()) {
        ERROR("Failed to copy DLL to temp directory");
        return 1;
    }
...
fs::path findDLL() {
    ...
    INFO("Searching for DLLs in: " + moduleDir.string());

    // Search for DLL in module directory
...
fs::path getTempDllPath(const fs::path& dllPath) {
    ...
    return tempDir / dllPath.filename();
}
fs::path copyDllToTemp(const fs::path& dllPath) {
    ...
    try {
        // Make sure the temp file doesn't exist (could cause issues if it's locked)
        if (fs::exists(tempDllPath)) {
            fs::remove(tempDllPath);
        }
        
        // Copy to temp directory
        fs::copy_file(dllPath, tempDllPath, fs::copy_options::overwrite_existing);
        INFO("DLL copied to: " + tempDllPath.string());
    } catch (const fs::filesystem_error& e) {
        ERROR("Failed to copy DLL: " + std::string(e.what()));
        return {};
    }
